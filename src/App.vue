...
function persistState() {
  localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({ config, activeSection: activeSection.value }),
  );
}
...
async function readConfig(event: Event) {
  ...
  const parsed = parse(await file.text(), errors, {
...
<template>
  <main class="settings-shell">
    <header class="topbar">
      <div class="brand">
        <span class="brand-mark">›_</span><span>Fastfetch Config Editor</span>
      </div>
      <div class="topbar-actions">
        <button class="secondary-button" type="button" @click="newConfig">
          New
        </button>
        <button class="secondary-button" type="button" @click="importConfig">
          Import JSONC
        </button>
        <button class="primary-button" type="button" @click="downloadConfig">
...
.workspace {
  flex: 1;
  display: grid;
  grid-template-columns: 238px minmax(450px, 1fr) minmax(320px, 0.75fr);
  min-height: 0;
  overflow: hidden;
  transition: grid-template-columns 0.2s;
}
.workspace.sidebar-collapsed {
  grid-template-columns: 52px minmax(450px, 1fr) minmax(320px, 0.75fr);
}
.workspace.preview-collapsed {
  grid-template-columns: 238px 1fr;
}
.workspace.sidebar-collapsed.preview-collapsed {
